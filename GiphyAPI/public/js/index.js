...
{
    // add event listeners of hovering over the newly added gifs
...
function addButtonFunctionality(button, current)
{
    button.addEventListener("click", function(){
        ...
        {
            // making an AJAX request to the server, via path removeFavorite. 
...
{
    // modifying the lookup path, based on what the current pathname is of the webpage
    ...
    // getting gifs based on the look string from the server.
    fetch(look)
    .then( (r)=>r.json() )
    .then( (r) => {
...
// if at home directory, then look up trending gifs.
...
else if(document.location.pathname === "/favorite")
{
    ...
    // get the information form the favorites
    fetch("/getFavorites")
    .then( (r)=>r.json() )
    .then( (r)=>{
